...
class m180121_120402_create_rbac_tables extends Migration
{
    ...
    public function up()
    {
        ...
        $story = $authManager->createRole('story');
        $diller = $authManager->createRole('diller');
        $market = $authManager->createRole('market');
        $client = $authManager->createRole('client');
        $authManager->add($admin); //admin
        $authManager->add($company); //postavshik
        $authManager->add($diller); //diller
        $authManager->add($market); //magazin
        $authManager->add($client); //client
        $authManager->add($story); //story
        $authManager->addChild($admin, $company);
        $authManager->addChild($company, $story);
        $authManager->addChild($company, $diller);
        $authManager->addChild($diller, $market);
        $authManager->addChild($market, $client);
